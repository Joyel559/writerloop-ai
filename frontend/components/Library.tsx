...
import { extractCbzCoverDataUrl, isCbzFilename } from '@/lib/cbz';
...
function stripBookExtension(fileName: string) {
  return fileName.replace(/\.(txt|pdf|epub|md|cbz)$/i, '');
}
...
function createFallbackCover(title: string, author?: string) {
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="600" height="900" viewBox="0 0 600 900">
      ...
      <text x="72" y="810" fill="${palette.subtleText}" font-size="20" font-family="Inter, Arial, sans-serif" letter-spacing="4">WRITERLOOP AI</text>
    </svg>
  `;
...
export function Library({ onSelectBook }: LibraryProps) {
  ...
  async function extractBookMetadata(file: File, type: Book['type'], content: string | ArrayBuffer) {
    ...
    try {
      ...
      if (type === 'cbz') {
        try {
          cover = await extractCbzCoverDataUrl((content as ArrayBuffer).slice(0));
        } catch (coverError) {
          console.warn('Failed to extract CBZ cover', coverError);
        }
      }
  ...
  const onDrop = async (acceptedFiles: File[]) => {
    for (const file of acceptedFiles) {
      ...
      const type: Book['type'] = lower.endsWith('.epub')
        ? 'epub'
        : lower.endsWith('.pdf')
          ? 'pdf'
          : isCbzFilename(lower)
  ...
  return (
    <div className="min-h-screen bg-transparent pb-20 font-sans text-ink selection:bg-sea/20">
      ...
      {showWebModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="absolute left-0 top-0 h-1 w-full bg-gradient-to-r from-sea to-amber" />
            ...
            <motion.input
              ...
              className="w-full rounded-xl border border-slate-300 px-3 py-2.5 transition-all focus:border-sea/40 focus:outline-none focus:ring-4 focus:ring-sea/10"
            />
            <motion.input
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: 0.18 }} className="flex justify-end gap-2 pt-1">
              ...
              <button
                ...
                className="rounded-xl bg-gradient-to-r from-sea to-amber px-4 py-2 text-white shadow-[0_14px_30px_-16px_rgba(21,122,110,0.65)] transition-all duration-200 hover:-translate-y-0.5 hover:opacity-95"
              >
                Add
              </button>
            </motion.div>
