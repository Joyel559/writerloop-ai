...
import { Bot, MessageSquarePlus, X, Check } from 'lucide-react';
...
export function SelectionMenu({ onHighlight, onAskAi, onNote, onClose, position, showAiAction = false, theme = 'light' }: SelectionMenuProps) {
    ...
    <AnimatePresence>
      ...
      >
        <div className={cn('flex items-center gap-1 border-r pr-2 mr-2', isDark ? 'border-slate-700' : isEInk ? 'border-neutral-600' : isSepia ? 'border-[#cbb99a]' : 'border-gray-200')}>
          {(['green', 'red'] as const).map((color) => (
            <button
              key={color}
              onClick={() => onHighlight(color)}
              className="w-6 h-6 rounded-full hover:scale-110 transition-transform border border-black/10"
              style={{ backgroundColor: color === 'green' ? '#bbf7d0' : '#fecaca' }}
            />
          ))}
        </div>
        {showAiAction && onAskAi && (
          <button onClick={onAskAi} className={cn('inline-flex items-center gap-1.5 px-2 py-1.5 rounded-full text-xs font-semibold transition-colors', iconHover, iconMuted)} title="Explain with AI">
