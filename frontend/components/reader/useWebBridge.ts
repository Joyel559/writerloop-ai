...
const WEB_BLOCKED_HOSTS_KEY = 'writerloop.web.blocked-hosts.v1';
...
export function useWebBridge({
  ...
  useEffect(() => {
    ...
    const postBridgeAck = (ok: boolean, reqId?: string, action?: string, error?: string, detail?: string) => {
      ...
      targetWindow.postMessage({ source: 'writerloop-host', type: 'host-ack', protocolVersion: '2.0.0', ok, reqId, action, error, detail, ts: Date.now() }, '*');
    };
    ...
    const handleBridgeMessage = (event: MessageEvent) => {
      ...
      if (!payload || payload.source !== 'writerloop-web-bridge') return;
    ...
    const helloTimer = window.setInterval(() => {
      ...
      frameWindow.postMessage({ source: 'writerloop-host', type: 'host-hello', protocolVersion: '2.0.0', ts: Date.now() }, '*');
    }, 1500);
