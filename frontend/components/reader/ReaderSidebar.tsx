...
interface ReaderSidebarProps {
  ...
  onExplainHighlight: (highlight: Highlight) => void;
      ...
      <div className="w-[350px] h-full flex flex-col">
        <div className={cn('grid grid-cols-5 border-b', styles.bg, styles.sidebarBorder)}>
          {(['chat', 'contents', 'highlights', 'bookmarks', 'search'] as SidebarTab[]).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={cn('py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors relative', activeTab === tab ? 'text-[#157a6e]' : cn(styles.tabInactiveText, styles.tabHoverText))}>
              ...
              {activeTab === tab && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#157a6e]" />}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-hidden relative">
          <div className={cn('absolute inset-0 transition-all duration-300 ease-out', activeTab === 'chat' ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0 pointer-events-none')}>
            <ChatSidebar
              book={book}
              currentTextContext={currentContext || 'Select text or image with Arrow mode to ask AI.'}
              quotedText={quotedText}
              selectedImage={selectedImage}
              autoExplainNonce={autoExplainNonce}
                    ...
                    return (
                      <div key={root.id}>
                        <div className="flex items-center gap-1">
                          ...
                          <button ref={isActive ? activeMdItemRef : null} onClick={() => scrollToHeading(root.id)} className={cn('flex-1 text-left py-2 px-2 rounded-lg text-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_24px_-20px_rgba(15,23,42,0.4)]', isActive ? 'bg-[#157a6e]/10 text-[#157a6e] font-medium' : styles.listItemHoverBg)}>
                            {root.text}
                          </button>
                        </div>
          ...
          <div className={cn('absolute inset-0 transition-all duration-300 ease-out flex flex-col', activeTab === 'bookmarks' ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0 pointer-events-none', styles.bg)}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {bookmarks.map((bookmark) => {
                ...
                return (
                  <div key={bookmark.id} className={cn('p-3 rounded-xl text-sm border transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_14px_30px_-22px_rgba(15,23,42,0.45)]', styles.noteCardBg, styles.noteCardBorder, isFocused && 'ring-2 ring-[#157a6e]/40 border-[#157a6e]/30')}>
                    {isEditing ? (
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <button onClick={() => { onRenameBookmark(bookmark.id, bookmarkDraft); setEditingBookmarkId(null); }} className="text-xs px-2 py-1 rounded-md bg-[#157a6e] text-white transition-all duration-200 hover:-translate-y-0.5">Save</button>
                        </div>
                      </div>
                    ) : (
                      <>
