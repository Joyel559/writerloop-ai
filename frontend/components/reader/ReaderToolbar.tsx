import { ArrowLeft, Bookmark, Bot, Highlighter as HighlightIcon, List, Minus, Monitor, Moon, MousePointer2, PanelLeft, PanelRight, Plus, ScrollText, Sun, Type } from 'lucide-react';
...
interface ReaderToolbarProps {
  ...
  setAiArrowEnabled: React.Dispatch<React.SetStateAction<boolean>>;
  onBack: () => void;
}
      ...
      <div className="flex items-center gap-2">
        ...
        <div className={cn('hidden md:flex items-center rounded-full p-1 mr-2 transition-all duration-200 hover:-translate-y-0.5', styles.toolbarGroupBg)} role="radiogroup" aria-label="Reading mode">
          ...
          <button
            ...
            title={disablePagedMode ? 'This format supports Scroll mode only' : 'Paged'}
            aria-label={disablePagedMode ? 'This format supports Scroll mode only' : 'Paged'}
        ...
        <button
          onClick={() => setAiArrowEnabled((v) => !v)}
          className={cn(
            'rounded-full px-3 py-2 transition-all duration-200 inline-flex items-center gap-1.5 text-xs font-semibold',
            aiArrowEnabled
              ? 'bg-gradient-to-r from-[#157a6e] to-[#f2994a] text-white shadow-[0_10px_22px_-14px_rgba(21,122,110,0.8)]'
              : styles.toolbarHoverBg,
          )}
          title={aiArrowEnabled ? 'Arrow AI On' : 'Arrow AI Off'}
          aria-pressed={aiArrowEnabled}
        >
        ...
        <button
          ...
          className={cn('p-2 rounded-full transition-all duration-200', showSidebar && activeTab === 'contents' ? 'bg-gradient-to-r from-[#157a6e] to-[#f2994a] text-white shadow-[0_10px_22px_-14px_rgba(21,122,110,0.8)]' : styles.toolbarHoverBg)}
          title="Contents"
        >
        ...
        <button
          ...
          className={cn('p-2 rounded-full transition-all duration-200 flex items-center gap-2', showSidebar && activeTab !== 'contents' ? 'bg-gradient-to-r from-[#157a6e] to-[#f2994a] text-white shadow-[0_10px_22px_-14px_rgba(21,122,110,0.8)]' : styles.toolbarHoverBg)}
        >
