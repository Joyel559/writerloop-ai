...
type ReaderHighlight = { text: string; color: HighlightColor; page?: number };
...
const PAGE_GAP = 16;
const PDF_HL_BG: Record<HighlightColor, string> = {
...
export function ReaderPdfView({
  ...
  const getPreparedHighlights = (pageNumber: number): Array<ReaderHighlight & { candidates: string[] }> => {
    return highlights
      .filter((h: ReaderHighlight) => (h.page ? h.page === pageNumber : true))
      .map((h: ReaderHighlight): ReaderHighlight & { candidates: string[] } => ({ ...h, candidates: buildHighlightCandidates(h.text || '') }))
  ...
  const findBestHighlightedSpan = (pageContainer: HTMLElement, pageNumber: number) => {
    const spans = Array.from(pageContainer.querySelectorAll('.textLayer span[data-writerloop-hl="1"]')) as HTMLSpanElement[];
  ...
  const applyPdfHighlightOverlay = (pageNumber: number) => {
    ...
    spans.forEach((span) => {
      if (span.dataset.writerloopHl === '1') {
        span.style.backgroundColor = '';
        span.style.borderRadius = '';
        span.style.boxShadow = '';
        delete span.dataset.writerloopHl;
      }
    });

    const prepared: Array<ReaderHighlight & { candidates: string[] }> = getPreparedHighlights(pageNumber);
    ...
    spans.forEach((span) => {
      ...
      let bestColor: HighlightColor | null = null;
      let bestScore = -1;

      prepared.forEach((item) => {
        const score = getSpanMatchScore(token, item.candidates);
        if (score > bestScore) {
          bestColor = item.color as HighlightColor;
          bestScore = score;
        }
      });

      if (!bestColor || bestScore < 6) return;
      span.style.backgroundColor = PDF_HL_BG[bestColor];
      span.style.borderRadius = '2px';
      span.style.boxShadow = `0 0 0 1px ${PDF_HL_BG[bestColor]}`;
      span.dataset.writerloopHl = '1';
    });
  };
  ...
  return (
    <div ref={rootRef} className="h-full w-full overflow-auto flex flex-col items-center pt-8 pb-20">
      <Document
        key={`${fileKey}:${workerFallbackNonce}`}
        file={stableFile}
        ...
        onLoadError={(error) => {
          const message = error?.message || 'Failed to load PDF file.';
          const isVersionMismatch = message.includes('does not match Worker version');
          if (isVersionMismatch && workerFallbackNonce === 0) {
            pdfjs.GlobalWorkerOptions.workerSrc = `https://unpkg.com/pdfjs-dist@${pdfjs.version}/legacy/build/pdf.worker.min.mjs`;
            setPdfHint('Detected stale worker cache. Retrying with version-matched worker...');
            setWorkerFallbackNonce(1);
            setIsLoadingDocument(true);
            return;
          }
