...
import { ReaderCbzView } from './ReaderCbzView';
...
interface ReaderMainContentProps {
  ...
  setLayoutWidth: Dispatch<SetStateAction<ReaderLayoutWidth>>;
  ...
  setSidebarPositionMode: Dispatch<SetStateAction<SidebarPositionMode>>;
  aiArrowEnabled: boolean;
  setAiArrowEnabled: Dispatch<SetStateAction<boolean>>;
  onBack: () => void;
  onAddBookmark: () => void;
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative min-w-0">
      <ReaderToolbar
        ...
        disablePagedMode={p.isEpub || p.isWeb || p.isCbz || p.book.type === 'pdf'}
        ...
        aiArrowEnabled={p.aiArrowEnabled}
        setAiArrowEnabled={p.setAiArrowEnabled}
        onBack={p.onBack}
      />

      <div className="flex-1 relative overflow-hidden" id="pdf-container">
        <SelectionMenu
          position={p.selectionPos}
          theme={p.theme}
          showAiAction={p.aiArrowEnabled}
          onAskAi={p.onExplainSelection}
          onClose={() => p.setSelectionPos(null)}
          onHighlight={(color) => p.addHighlight(color)}
          onNote={() => p.setShowNoteDialog(true)}
        />
        <NoteDialog isOpen={p.showNoteDialog} theme={p.theme} onClose={() => p.setShowNoteDialog(false)} onSave={(note) => p.addHighlight('green', note)} />
            ...
            >
              <div className="flex flex-col md:flex-row gap-2">
                <input
                  ...
                  className={cn('flex-1 rounded-lg px-3 py-2 text-sm outline-none border transition-all duration-200 focus:ring-4 focus:ring-[#157a6e]/10', p.styles.bg, p.styles.text, p.styles.sidebarBorder)}
                />
                <button
                  onClick={() => {
                  ...
                  className="px-3 py-2 rounded-lg bg-gradient-to-r from-[#157a6e] to-[#f2994a] text-white text-sm shadow-[0_12px_24px_-14px_rgba(21,122,110,0.8)] transition-all duration-200 hover:-translate-y-0.5 hover:opacity-95"
                >
                  Load URL
                </button>
              </div>
