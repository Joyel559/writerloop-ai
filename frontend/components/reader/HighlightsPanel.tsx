...
import { Bot, Download, Highlighter as HighlightIcon, Pencil, Trash2 } from 'lucide-react';
...
export function HighlightsPanel({
  ...
  const [highlightColorDraft, setHighlightColorDraft] = useState<HighlightColor>('green');
      ...
      <div className={cn('border-b px-4 py-3 space-y-3', styles.sidebarBorder)}>
        <div className="flex flex-wrap gap-2">
          {HIGHLIGHT_FILTER_OPTIONS.map((option) => {
            ...
            return (
              <button key={option.key} onClick={() => onHighlightColorFilterChange(option.key)} className={cn('px-2.5 py-1 rounded-full text-xs border transition-all duration-200 hover:-translate-y-0.5 inline-flex items-center gap-1.5', colorBadge, isActive ? 'ring-2 ring-[#157a6e]/35 border-[#157a6e]/30' : 'opacity-80 hover:opacity-100')}>
                ...
                >
                  ...
                  {isEditing ? (
                    <div className="space-y-2 ml-4">
                      <select value={highlightColorDraft} onChange={(e) => setHighlightColorDraft(e.target.value as HighlightColor)} className="w-full rounded-md border px-2 py-1 text-xs"><option value="green">Green</option><option value="red">Red</option></select>
                      ...
                      <div className="flex justify-end gap-2"><button onClick={() => setEditingHighlightId(null)} className={cn('text-xs px-2 py-1 rounded-md transition-all duration-200 hover:-translate-y-0.5', styles.listItemHoverBg)}>Cancel</button><button onClick={() => { onUpdateHighlight(hl.id, { color: highlightColorDraft, note: highlightNoteDraft.trim() || undefined }); setEditingHighlightId(null); }} className="text-xs px-2 py-1 rounded-md bg-[#157a6e] text-white transition-all duration-200 hover:-translate-y-0.5">Save</button></div>
                    </div>
                  ) : (
                    <>
                      {hl.note && <div className="ml-4 pl-2 border-l-2 border-[#157a6e]"><div className={cn('prose prose-sm max-w-none [&_p]:my-1 [&_li]:my-0.5 [&_code]:px-1 [&_code]:py-0.5 [&_code]:rounded [&_code]:bg-black/10')}><ReactMarkdown>{hl.note}</ReactMarkdown></div></div>}
                      <div className="mt-2 flex justify-end gap-2"><button onClick={() => onExplainHighlight(hl)} className={cn('inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md bg-[#157a6e] text-white transition-all duration-200 hover:-translate-y-0.5')}><Bot className="w-3.5 h-3.5" />Explain</button><button onClick={() => { setEditingHighlightId(hl.id); setHighlightColorDraft(hl.color); setHighlightNoteDraft(hl.note || ''); }} className={cn('inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md transition-all duration-200 hover:-translate-y-0.5', styles.listItemHoverBg)}><Pencil className="w-3.5 h-3.5" />Edit</button><button onClick={() => onDeleteHighlight(hl.id)} className={cn('inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md transition-all duration-200 hover:-translate-y-0.5', styles.listItemHoverBg)}><Trash2 className="w-3.5 h-3.5" />Delete</button></div>
                    </>
                  )}
                </motion.div>
              );
