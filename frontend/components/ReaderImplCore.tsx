...
const AI_ARROW_STORAGE_KEY = 'writerloop.reader.ai-arrow-enabled';
...
export function Reader({ book, onBack }: ReaderProps) {
  ...
  const [aiArrowEnabled, setAiArrowEnabled] = useState<boolean>(() => {
    if (typeof window === 'undefined') return false;
    return window.localStorage.getItem(AI_ARROW_STORAGE_KEY) === '1';
  });
  ...
  const getNormalizedProgress = useCallback(() => {
    ...
    if ((book.type === 'pdf' || book.type === 'cbz') && typeof location === 'number' && numPages > 0) {
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    window.localStorage.setItem(AI_ARROW_STORAGE_KEY, aiArrowEnabled ? '1' : '0');
  }, [aiArrowEnabled]);
  ...
  const handleExplainHighlight = useCallback((highlight: Highlight) => {
    const selected = highlight.text?.trim();
    if (!selected) return;

    setSelectedImage(null);
    setQuotedText(selected);
    setCurrentContext(highlight.note?.trim() || selected);
    setActiveTab('chat');
    setShowSidebar(true);
    triggerAutoExplain();
  }, [setActiveTab, setCurrentContext, setQuotedText, setSelectedImage, setShowSidebar, triggerAutoExplain]);
  ...
  const { effectiveWpm } = useReadingSpeedEstimator({
    ...
    content:
      book.content ??
      (book.type === 'pdf' || book.type === 'epub' || book.type === 'cbz'
        ? new ArrayBuffer(0)
        : ''),
    location,
    layoutTick,
  ...
  const estimatedReadingMinutes = useMemo(() => {
    ...
    if (book.type === 'pdf' || book.type === 'epub' || book.type === 'cbz') {
      const pages = book.type === 'epub' ? Math.max(1, epubToc.length || 1) : Math.max(1, numPages || 1);
  ...
  return (
    <div className={cn('flex h-screen w-full overflow-hidden transition-colors duration-300', styles.bg, styles.text)}>
      ...
      <ReaderMainContent
        ...
        aiArrowEnabled={aiArrowEnabled}
        setAiArrowEnabled={setAiArrowEnabled}
        onBack={handleBack}
        onAddBookmark={handleAddBookmark}
        selectionPos={selectionPos}
        setSelectionPos={() => setSelectionPos(null)}
        onExplainSelection={askAiForCurrentSelection}
